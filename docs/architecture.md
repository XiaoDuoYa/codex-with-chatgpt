...
| `mcp/` | McpServer with 9 read-only tools plus the separately scoped `submit_plan` inbox action; stateless Streamable HTTP transport (fresh server per request, JSON responses) |
...
| `plans/` | One-time digest-bound authorization, staged-release revalidation, and create-only owner-controlled plan inbox |
