...
import { withBridgeLifecycleLock as withWorkspaceBridgeLifecycleLock } from "../session/lock.js";
...
/**
 * Serialize a bridge lifecycle transition for one workspace.
 */
export async function withBridgeLifecycle<T>(workspaceRoot: string, action: () => Promise<T>): Promise<T> {
 const workspace = new Workspace(workspaceRoot);
 return withWorkspaceBridgeLifecycleLock(workspace.id, action);
}

async function ensureBridgeUnserialized(
...
/**
 * Ensure a bridge while the caller already owns the lifecycle lock.
 */
export async function ensureBridgeWithinLifecycleLock(
 workspaceRoot: string,
 opts: { port?: number } = {}
): Promise<EnsureBridgeResult> {
 return ensureBridgeUnserialized(workspaceRoot, opts);
}

/**
 * Ensure a bridge is running for the workspace.
 */
export async function ensureBridge(workspaceRoot: string, opts: { port?: number } = {}): Promise<EnsureBridgeResult> {
 return withBridgeLifecycle(workspaceRoot, () => ensureBridgeWithinLifecycleLock(workspaceRoot, opts));
}


/**
 * Listen to an existing bridge's admin API.
 */
...
async function stopBridgeUnserialized(workspaceRoot: string): Promise<boolean> {
 const workspace = new Workspace(workspaceRoot);
...
/**
 * Stop a bridge while the caller already owns the lifecycle lock.
 */
export async function stopBridgeWithinLifecycleLock(workspaceRoot: string): Promise<boolean> {
 return stopBridgeUnserialized(workspaceRoot);
}

/**
 * Stop the bridge for the workspace.
 */
export async function stopBridge(workspaceRoot: string): Promise<boolean> {
 return withBridgeLifecycle(workspaceRoot, () => stopBridgeWithinLifecycleLock(workspaceRoot));
}
