...
连接器元数据变更后，旧 Chat 可能不再提供该应用，但同一 Project 的新 Chat
仍可选择。Skill 会在旧 Chat 的应用选择器中使用真实键盘事件搜索完整名称，并等待
异步候选；推荐列表和页面里的普通文字都不算可用证据。只有旧 Chat 确认没有可选
连接器、且同一 Project 的新 Chat 入口确认已选应用及所需工具，才复用原物理标签页
创建替代 Chat，不要求特定的内联标签形态。探测最多使用一个自建隐藏辅助页，不发送
消息，完成后关闭该辅助页；替换 claim 成功前不导航原进度页。最多复查一次仍不明确时，
保留原绑定并报告诊断，不无限检查，也不借此重试平台已拒绝的任务。
