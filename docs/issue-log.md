...
- 本地验证：38 个测试文件、522 项测试全部通过，类型检查、构建、Skill 校验通过。模拟同一任务运行九十分钟后，经 MCP SDK 成功回传 BLOCKED；另覆盖 CLI 续期、短租约提前检查、旧观察重放、过期/撤销授权拒绝续期、并发回执优先保留及续期记录完整性。九十分钟为本地模拟时间，不是实际 ChatGPT 网页长任务验收。
